#!/usr/bin/env python3
"""Run Jan-Apr 2024 with CO2 price, MCs, ramping, no expansion, no ICs."""
...
# ── Solve: Jan-Apr 2024 ──────────────────────────────────────────────
start=pd.Timestamp('2024-01-01'); end=pd.Timestamp('2024-04-30 23:00')
snaps=n.snapshots[(n.snapshots>=start)&(n.snapshots<=end)]
print(f'\nSolving {len(snaps)} snapshots ({len(snaps)/24:.0f} days, {start.date()} to {end.date()})...')
...
print(f'Status: {res[0]} | Objective: {n_sub.objective:.2e} EUR')

# ── Save ──────────────────────────────────────────────────────────────
sdir=f'{ROOT}/solved_networks/04_solve_diagnostic'
os.makedirs(sdir,exist_ok=True)
n_sub.export_to_netcdf(f'{sdir}/solved_2024_4m_co2.nc')
print('Saved.')
...
for m in range(1,5):
    ...
    print(f'\n  Month {m}: Mean={m_p.stack().mean():.1f}, Max={m_p.stack().max():.1f}, Min={m_p.stack().min():.1f}')
...
print(f'OMIE   >50:  {(omie_period>50).sum()}/{len(omie_period)} hours ({(omie_period>50).mean()*100:.1f}%)')
print(f'Model >100: {(model_mean>100).sum()}/{len(model_mean)} hours ({(model_mean>100).mean()*100:.1f}%)')
print(f'OMIE   >100: {(omie_period>100).sum()}/{len(omie_period)} hours ({(omie_period>100).mean()*100:.1f}%)')

# Monthly comparison
print('\n=== MONTHLY COMPARISON ===')
for m in range(1,5):
    m_model = model_mean[model_mean.index.month == m]
    m_omie = omie_period[omie_period.index.month == m]
    print(f'  Month {m}: Model mean={m_model.mean():.1f}, OMIE mean={m_omie.mean():.1f}, '
          f'Model max={m_model.max():.1f}, OMIE max={m_omie.max():.1f}')
...
total_dispatch=g.sum().groupby(n_sub.generators.carrier).sum() / 1e6  # TWh
print('\n=== TOTAL GENERATION (TWh) ===')
for car, val in total_dispatch.sort_values(ascending=False).items():
    print(f'  {car:20s}: {val:.2f} TWh')

# Storage dispatch
su_dispatch=n_sub.storage_units_t.p
su_dispatch_pos=su_dispatch[su_dispatch>0].sum().groupby(n_sub.storage_units.carrier).sum() / 1e6
...
# Inflow
inflow = n_sub.storage_units_t.inflow[es_hyd_cols]
...
real_period = real[(real.index >= start) & (real.index <= end)]

# Model daily dispatch by carrier
model_gen = n_sub.generators_t.p
model_daily = model_gen.groupby(n_sub.generators.carrier, axis=1).sum().resample('D').sum() / 1000  # GWh/day
...
for m in range(1,5):
    ...
    m_real_hyd = real_period.loc[real_period.index.month == m, 'Hidráulica'].mean()
    m_model_ccgt = model_daily.loc[model_daily.index.month == m, 'CCGT'].mean() if 'CCGT' in model_daily.columns else 0
    m_real_ccgt = real_period.loc[real_period.index.month == m, 'Ciclo combinado'].mean()
    hyd_ratio = m_model_hyd / m_real_hyd if m_real_hyd > 0 else 0
    ccgt_ratio = m_model_ccgt / m_real_ccgt if m_real_ccgt > 0 else 0
    print(f'  Month {m:1d}     {m_model_hyd:8.0f} GWh  {m_real_hyd:8.0f} GWh  {hyd_ratio:6.2f}x  {m_model_ccgt:8.0f} GWh  {m_real_ccgt:8.0f} GWh  {ccgt_ratio:6.2f}x')
...
# Top: full timeseries
ax1.plot(model_mean.index, model_mean.values, label='Model (AC, CO2=65)', color='steelblue', lw=0.8)
ax1.plot(omie_period.index, omie_period.values, label='OMIE 2024', color='darkorange', lw=0.5, alpha=0.6)
ax1.axhline(y=100, color='red', ls='--', lw=0.5, alpha=0.5)
ax1.set_ylabel('EUR/MWh')
ax1.set_title('ES Price: Model (AC buses, CO₂=€65/t) vs Real OMIE — Jan-Apr 2024')
...
ax2.plot(np.arange(1, len(omie_pdc)+1)/len(omie_pdc)*100, omie_pdc,
         label=f'OMIE (mean={omie_period.mean():.1f}, max={omie_pdc.max():.0f})', color='darkorange', alpha=0.7)
...
plt.tight_layout()
plt.savefig(f'{sdir}/4m_price_comparison.png', dpi=150, bbox_inches='tight')
print(f'  Saved: 4m_price_comparison.png')

# 5b. Dispatch comparison
fig, axes = plt.subplots(2, 1, figsize=(16, 10))

# Top: Model dispatch stack
model_dispatch = model_gen.groupby(n_sub.generators.carrier, axis=1).sum()
...
axes[0].set_title('Model Dispatch by Carrier (Weekly Average)')
...
real_hydro_daily = real_period['Hidráulica']
real_ccgt_daily = real_period['Ciclo combinado']

axes[1].plot(model_hydro_daily.index, model_hydro_daily.values, 'b-', lw=1, label='Model Hydro')
axes[1].plot(real_hydro_daily.index, real_hydro_daily.values, 'b--', lw=0.7, alpha=0.6, label='Real Hydro')
axes[1].plot(model_ccgt_daily.index, model_ccgt_daily.values, 'orange', lw=1, label='Model CCGT')
axes[1].plot(real_ccgt_daily.index, real_ccgt_daily.values, 'orange', ls='--', lw=0.7, alpha=0.6, label='Real CCGT')
...
plt.tight_layout()
plt.savefig(f'{sdir}/4m_dispatch_comparison.png', dpi=150, bbox_inches='tight')
print(f'  Saved: 4m_dispatch_comparison.png')

# 5c. Monthly bar chart
fig, axes = plt.subplots(1, 2, figsize=(14, 5))

months = ['Jan', 'Feb', 'Mar', 'Apr']
model_hyd_monthly = [model_hyd_daily[model_hyd_daily.index.month == m].sum() for m in range(1,5)]
real_hyd_monthly = [real_period.loc[real_period.index.month == m, 'Hidráulica'].sum() for m in range(1,5)]
model_ccgt_monthly = [model_daily.loc[model_daily.index.month == m, 'CCGT'].sum() if 'CCGT' in model_daily.columns else 0 for m in range(1,5)]
real_ccgt_monthly = [real_period.loc[real_period.index.month == m, 'Ciclo combinado'].sum() for m in range(1,5)]
...
axes[0].set_xticks(x); axes[0].set_xticklabels(months)
...
plt.tight_layout()
plt.savefig(f'{sdir}/4m_monthly_bars.png', dpi=150, bbox_inches='tight')
print(f'  Saved: 4m_monthly_bars.png')
